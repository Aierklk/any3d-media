...
| `pnpm render <id>` | Remotion 渲染最终 `out/<id>/video/<id>.mp4` |
| `pnpm qa <id>` | 提取关键帧送 glm-5v-turbo 检查 → `qa-report.json` |
| `pnpm card <id> <style> [lang]` | 卡片生成 + 渲染 → `out/<id>/images/cards/*.png` |
...
any3d-media/
├── capture/                    # 录制阶段
│   ├── targets/                # 各工具的录制流程（每个 id 一个文件）
│   │   └── model-compression.ts
│   ├── flow.ts                 # 录制主流程（调度 recorder + anchors）
│   ├── recorder.ts             # Playwright 录制器（webm + 截帧）
│   ├── anchors.ts              # anchor 测量与 resolvedShots 生成
│   ├── card-shots.ts           # 卡片截图：重走 flow，按 anchor 截核心元素
│   └── types.ts                # AnchorMap、RecorderOptions 等类型
├── narration/
│   └── tts.ts                  # edge-tts 生成旁白 + 时序 JSON
├── captions/
│   └── build.ts                # 由旁白时序切分为 ≤12 字 cue
├── src/                        # Remotion 视频合成
│   ├── index.ts                # 入口（bundle → selectComposition → renderMedia）
│   ├── Root.tsx                # Composition 注册，读取 captions + captures
│   ├── PromoVideo.tsx          # 视频主组件
│   ├── VirtualCamera.tsx       # 虚拟运镜（cubic bezier 插值）
│   └── Captions.tsx            # 字幕渲染组件
├── scripts/                    # CLI 入口脚本
│   ├── run-all.ts              # pnpm all — 完整流水线
│   ├── run-cards.ts            # pnpm all:cards — 仅卡片流水线
│   ├── render.ts               # pnpm render — Remotion 渲染
│   └── probe-dom.ts            # DOM 探查辅助工具
├── card/                       # 社交卡片生成器
│   ├── assets/                 # HTML 模板 + WebGL 脚本
│   │   ├── template-editorial-card.html
│   │   ├── template-swiss-card.html
│   │   └── magazine-bg-webgl.js
│   ├── generate.ts             # 拼装卡片 HTML
│   ├── render.ts               # 截图生成 PNG
│   ├── specs.ts                # 卡片规格 + 双语文案
│   └── types.ts                # CardSpec、Poster 等类型
├── qa/
│   └── visual-qa.ts            # ffmpeg 抽帧 → glm-5v-turbo 视觉质检
├── config/
│   └── video-targets.ts        # 中央注册表（视频目标定义）
├── fixtures/
│   └── sample.glb              # 示例 3D 模型（gitignore）
├── docs/
│   └── superpowers/plans/      # 规划文档
├── tmp/                        # 运行时产物（gitignore）
├── out/                        # 最终产物（gitignore）
├── homepage.html               # 首页 HTML
├── main.css                    # 全局样式
...
capture  →  tmp/<id>/frames/*.jpg + captures.json(resolvedShots=静态shots叠加上实测算子框) + capture.webm
        │
narrate  →  out/<id>/audio/narration.mp3 + narration-timing.json
        │
captions →  out/<id>/subtitles/captions.json（按时序把旁白切分为 ≤12 字的 cue）
        │
   src/Root.tsx  在注册 Composition 时读取 captions.json + captures.json，
   把 resolvedShots / captionCues 作为 defaultProps 注入 PromoVideo
        │
render   →  out/<id>/video/<id>.mp4（bundle src/index.ts → selectComposition → renderMedia）
        │
qa       →  ffmpeg 抽帧 → glm-5v-turbo → out/<id>/qa/qa-report.json
        │
frames   →  capture/card-shots.ts 重走 flow，按 anchor 截核心元素到 tmp/<id>/card/frames/*.png
        │
cards    →  generate.ts 用 guizang 模板拼 HTML → render.ts 逐 .poster 截图 → out/<id>/images/cards/*.png
```
